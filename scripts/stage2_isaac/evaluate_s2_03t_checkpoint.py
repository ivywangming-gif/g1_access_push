#!/usr/bin/env python3
"""Camera-free screening and two-camera replay for redesigned S2-03T."""

from __future__ import annotations

import argparse
import hashlib
import json
import math
import os
import shutil
import sys
import traceback
from pathlib import Path
from typing import Any

from g1_access_push.stage2.s2_03t_redesign_contract import (
    validate_campaign_execution_manifest,
)

ACTOR_OBSERVATION_DIM = 135
ACTOR_ACTION_DIM = 2
REQUIRED_EXECUTION_SOURCES = (
    "scripts/stage2_isaac/evaluate_s2_03t_checkpoint.py",
    "scripts/stage2_isaac/s2_03t_visual_recorder.py",
    "src/g1_access_push/sim/stage2/s2_03t_actions.py",
    "src/g1_access_push/sim/stage2/s2_03t_agent_cfg.py",
    "src/g1_access_push/sim/stage2/s2_03t_env.py",
    "src/g1_access_push/sim/stage2/s2_03t_env_cfg.py",
    "src/g1_access_push/sim/stage2/s2_03t_mdp.py",
    "src/g1_access_push/stage2/s2_03t_redesign_contract.py",
)
CRITIC_OBSERVATION_DIM = 175
MAXIMUM_EPISODE_STEPS = 1000


def write_json(path: Path, payload: object) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_name(f".{path.name}.tmp.{os.getpid()}")
    with temporary.open("w", encoding="utf-8") as stream:
        json.dump(payload, stream, indent=2, sort_keys=True, allow_nan=False)
        stream.write("\n")
        stream.flush()
        os.fsync(stream.fileno())
    os.replace(temporary, path)


def sha256_file(path: Path) -> str:
    digest = hashlib.sha256()
    with path.open("rb") as stream:
        for chunk in iter(lambda: stream.read(1024 * 1024), b""):
            digest.update(chunk)
    return digest.hexdigest()


parser = argparse.ArgumentParser()
parser.add_argument("--run-root", type=Path, required=True)
parser.add_argument("--reference", type=Path, required=True)
parser.add_argument("--reference-sha256", required=True)
parser.add_argument("--campaign-manifest", type=Path, required=True)
parser.add_argument("--checkpoint", default="NONE")
parser.add_argument("--checkpoint-sha256", default="NONE")
parser.add_argument("--checkpoint-iteration", type=int)
parser.add_argument("--controller-id", choices=("actor", "original_s2_03"), default="actor")
parser.add_argument("--visual-evidence", action="store_true")
parser.add_argument(
    "--visual-evidence-id",
    choices=(
        "CLEAN_UNTRAINED_ACTOR",
        "BEST_GAP_CHECKPOINT",
        "ORIGINAL_S2_03_CONTROLLER",
        "PRIMARY",
        "SUPPLEMENTAL",
    ),
)
parser.add_argument("--visual-frame-stride", type=int, default=2)
parser.add_argument(
    "--mode", choices=("baseline", "pilot", "screening", "formal", "preflight"), required=True
)
parser.add_argument("--development-seed", type=int, required=True)
from isaaclab.app import AppLauncher

AppLauncher.add_app_launcher_args(parser)
args = parser.parse_args()
RUN = args.run_root.resolve()
RUN.mkdir(parents=True, exist_ok=False)
reference_path = args.reference.resolve()
if not reference_path.is_file() or sha256_file(reference_path) != args.reference_sha256:
    raise SystemExit("PRECONTACT_REFERENCE_SHA_MISMATCH")
validate_campaign_execution_manifest(
    args.campaign_manifest.resolve(),
    stage_run_root=RUN,
    reference_path=reference_path,
    reference_sha256=args.reference_sha256,
    required_source_relatives=REQUIRED_EXECUTION_SOURCES,
)
checkpoint_path = None if args.checkpoint == "NONE" else Path(args.checkpoint).resolve()
if checkpoint_path is not None:
    if checkpoint_path.name == "model_1999.pt":
        raise SystemExit("MODEL_1999_FORBIDDEN")
    if not checkpoint_path.is_file() or sha256_file(checkpoint_path) != args.checkpoint_sha256:
        raise SystemExit("ACTOR_CHECKPOINT_SHA_MISMATCH")
if args.controller_id == "original_s2_03" and checkpoint_path is not None:
    raise SystemExit("ORIGINAL_CONTROLLER_MUST_NOT_LOAD_ACTOR_CHECKPOINT")
if args.controller_id == "original_s2_03" and not args.visual_evidence:
    raise SystemExit("ORIGINAL_CONTROLLER_IS_VISUAL_DIAGNOSTIC_ONLY")
if args.visual_evidence and not args.enable_cameras:
    raise SystemExit("VISUAL_EVIDENCE_REQUIRES_ENABLE_CAMERAS")
if args.visual_evidence and args.visual_evidence_id is None:
    raise SystemExit("VISUAL_EVIDENCE_ID_REQUIRED")
if args.visual_evidence_id == "CLEAN_UNTRAINED_ACTOR" and checkpoint_path is not None:
    raise SystemExit("CLEAN_UNTRAINED_ACTOR_MUST_NOT_LOAD_CHECKPOINT")
if (
    args.visual_evidence_id == "ORIGINAL_S2_03_CONTROLLER"
    and args.controller_id != "original_s2_03"
):
    raise SystemExit("ORIGINAL_VISUAL_ID_REQUIRES_ORIGINAL_CONTROLLER")
if (
    args.controller_id == "original_s2_03"
    and args.visual_evidence_id != "ORIGINAL_S2_03_CONTROLLER"
):
    raise SystemExit("ORIGINAL_CONTROLLER_VISUAL_ID_MISMATCH")
if args.mode in {"screening", "formal"} and checkpoint_path is None:
    raise SystemExit("TRAINED_EVALUATION_REQUIRES_CHECKPOINT")
if (
    args.visual_evidence_id in {"BEST_GAP_CHECKPOINT", "PRIMARY", "SUPPLEMENTAL"}
    and checkpoint_path is None
):
    raise SystemExit("TRAINED_VISUAL_EVIDENCE_REQUIRES_CHECKPOINT")
write_json(RUN / "runner_status.json", {"status": "STARTING", "phase": "APP_LAUNCH"})
simulation_app = AppLauncher(args).app

import isaaclab.utils.math as math_utils  # noqa: E402
import torch  # noqa: E402
from agile.rl_env.rsl_rl import RslRlVecEnvWrapper  # noqa: E402
from PIL import Image  # noqa: E402
from rsl_rl.runners import OnPolicyRunner  # noqa: E402
from s2_03t_visual_recorder import VisualEvidenceRecorder  # noqa: E402

from g1_access_push.sim.stage2.s2_03t_agent_cfg import S203TPPORunnerCfg  # noqa: E402
from g1_access_push.sim.stage2.s2_03t_bootstrap import (  # noqa: E402
    MAXIMUM_ORIENTATION_CORRECTION_RAD,
    MAXIMUM_POSITION_CORRECTION_M,
    _clamp_norm,
    _target_pose_in_pelvis,
    derive_precontact_reference,
)
from g1_access_push.sim.stage2.s2_03t_env import S203TContactEnv  # noqa: E402
from g1_access_push.sim.stage2.s2_03t_env_cfg import S203TContactEnvCfg  # noqa: E402
from g1_access_push.sim.stage2.s2_03t_eval_cfg import S203TContactEvaluationEnvCfg  # noqa: E402
from g1_access_push.sim.stage2.s2_03t_mdp import runtime_state  # noqa: E402
from g1_access_push.sim.stage2.s2_03t_visual_cfg import S203TVisualEvidenceEnvCfg  # noqa: E402
from g1_access_push.stage2.s2_03_contract import (  # noqa: E402
    advance_rate_limited,
    classify_formal,
    load_config,
)


class VisualS203TContactEnv(S203TContactEnv):
    """Capture the terminal physics frame before ManagerBased auto-reset."""

    def __init__(self, *env_args, **env_kwargs) -> None:
        self.visual_terminal_callback = None
        super().__init__(*env_args, **env_kwargs)

    def _reset_idx(self, env_ids) -> None:
        if (
            self.visual_terminal_callback is not None
            and hasattr(self, "episode_length_buf")
            and not self._s2_03t_bootstrap_mode
        ):
            ids = torch.as_tensor(env_ids, dtype=torch.long, device=self.device)
            completed = ids[self.episode_length_buf[ids] > 0]
            if bool((completed == 0).any()):
                self.visual_terminal_callback()
        super()._reset_idx(env_ids)


def failure_reason(snapshot: dict[str, Any], terminal_record: dict[str, Any]) -> str:
    mapping = {
        "forbidden_non_palm_box_collision": "FORBIDDEN_BODY_BOX_COLLISION",
        "force_peak": "EXCESSIVE_CONTACT_IMPULSE",
        "palm_impulse": "EXCESSIVE_CONTACT_IMPULSE",
        "combined_impulse": "EXCESSIVE_CONTACT_IMPULSE",
        "force_rate": "EXCESSIVE_CONTACT_IMPULSE",
        "box_linear_speed": "OBJECT_TRANSLATION_DURING_ATTACH",
        "box_translation": "OBJECT_TRANSLATION_DURING_ATTACH",
        "box_angular_speed": "OBJECT_YAW_DURING_ATTACH",
        "box_yaw_change": "OBJECT_YAW_DURING_ATTACH",
        "base_excursion": "ROBOT_FALL",
        "root_height": "ROBOT_FALL",
        "root_tilt": "ROBOT_BAD_TILT",
        "arm_joint_margin": "JOINT_LIMIT_VIOLATION",
        "arm_torque": "TORQUE_LIMIT_VIOLATION",
        "nonfinite": "NONFINITE",
        "single_hand_timeout": "BILATERAL_CONTACT_TIMEOUT",
    }
    reasons = snapshot.get("termination_reasons", [])
    if reasons and reasons[0] == "contact_loss":
        left_streak = int(terminal_record["left_contact_loss_streak"])
        right_streak = int(terminal_record["right_contact_loss_streak"])
        return "RIGHT_CONTACT_LOST" if right_streak > left_streak else "LEFT_CONTACT_LOST"
    return (
        mapping.get(str(reasons[0]), "BILATERAL_CONTACT_TIMEOUT")
        if reasons
        else "BILATERAL_CONTACT_TIMEOUT"
    )


env = None
visual_recorder = None
scientific_status = "INVALID"
try:
    torch.manual_seed(args.development_seed)
    torch.cuda.manual_seed_all(args.development_seed)
    cfg = (
        S203TVisualEvidenceEnvCfg()
        if args.visual_evidence
        else S203TContactEvaluationEnvCfg()
        if args.enable_cameras
        else S203TContactEnvCfg()
    )
    cfg.seed = args.development_seed
    cfg.scene.num_envs = 1
    cfg.sim.device = args.device
    # The subclass has no camera dependency.  It is used for numeric screening
    # too so the final trace sample is captured before ManagerBased auto-reset.
    env = VisualS203TContactEnv(
        cfg=cfg,
        render_mode="rgb_array" if args.enable_cameras else None,
    )
    env.configure_contact_curriculum(enabled=False, maximum_level=0, fixed_gap_m=0.06)
    camera = env.scene["audit_camera"] if args.enable_cameras else None
    if camera is not None:
        camera.set_world_poses_from_view(
            torch.tensor([[-1.6, -2.2, 1.40]], device=env.device),
            torch.tensor([[0.45, 0.0, 0.70]], device=env.device),
        )
    if args.visual_evidence:
        env.scene["side_camera"].set_world_poses_from_view(
            torch.tensor([[0.55, -2.35, 0.92]], device=env.device),
            torch.tensor([[0.55, 0.0, 0.68]], device=env.device),
        )

    records: list[dict[str, Any]] = []
    transitions = [{"frame": -1, "state": "RESET", "reason": None}]
    contact_onsets: list[int | None] = [None, None]
    bilateral_onset = None
    previous_contacts = [False, False]
    contact_losses: list[list[int]] = [[], []]

    def append_record(state_name: str, metric, snapshot=None) -> dict[str, Any]:
        global bilateral_onset, previous_contacts
        frame = len(records)
        if snapshot is None:
            forces = [float(metric["forces"][0, 0]), float(metric["forces"][0, 1])]
            contacts = [bool(metric["contacts"][0, 0]), bool(metric["contacts"][0, 1])]
            gaps = [float(metric["gaps"][0, 0]), float(metric["gaps"][0, 1])]
            impulse = [float(value) for value in runtime_state(env).impulse[0]]
            values = {
                "force_rate": float(metric["force_rate"][0]),
                "box_translation": float(metric["box_translation"][0]),
                "box_yaw": abs(float(metric["box_yaw_change"][0])),
                "box_linear": float(metric["box_linear_speed"][0]),
                "box_angular": float(metric["box_angular_speed"][0]),
                "root_height": float(metric["root_height"][0]),
                "root_tilt": float(metric["root_tilt_deg"][0]),
                "joint_margin": float(metric["arm_joint_margin"][0]),
                "torque_ratio": float(metric["arm_torque_ratio"][0]),
                "finite": bool(metric["finite"][0]),
                "forbidden": bool(metric["failure_forbidden_non_palm_box_collision"][0]),
                "position_error": metric["palm_position_error"][0],
                "orientation_error": metric["palm_orientation_error"][0],
            }
        else:
            forces = list(snapshot["maximum_force_n"])
            contacts = [True, True] if snapshot["success"] else list(previous_contacts)
            gaps = list(snapshot["minimum_surface_gap_m"])
            impulse = list(snapshot["impulse_ns"])
            values = {
                "force_rate": 0.0,
                "box_translation": float(snapshot["maximum_box_translation_m"]),
                "box_yaw": float(snapshot["maximum_box_yaw_change_rad"]),
                "box_linear": 0.0,
                "box_angular": 0.0,
                "root_height": float(snapshot["minimum_root_height_m"]),
                "root_tilt": float(snapshot["maximum_root_tilt_deg"]),
                "joint_margin": float(snapshot["minimum_arm_joint_margin_rad"]),
                "torque_ratio": float(snapshot["maximum_arm_torque_ratio"]),
                "finite": bool(snapshot["finite"]),
                "forbidden": "forbidden_non_palm_box_collision" in snapshot["termination_reasons"],
                "position_error": torch.zeros((2, 3), device=env.device),
                "orientation_error": torch.zeros((2, 3), device=env.device),
            }
        for side in range(2):
            if contacts[side] and contact_onsets[side] is None:
                contact_onsets[side] = frame
            if previous_contacts[side] and not contacts[side]:
                contact_losses[side].append(frame)
        if all(contacts) and bilateral_onset is None:
            bilateral_onset = frame
        previous_contacts = contacts
        arm = env.action_manager.get_term("arm_residual")
        current_state = runtime_state(env)
        record = {
            "frame": frame,
            "time_s": (frame + 1) * float(env.step_dt),
            "fsm_state": state_name,
            "finite": values["finite"],
            "left_contact": contacts[0],
            "right_contact": contacts[1],
            "left_force_n": forces[0],
            "right_force_n": forces[1],
            "left_impulse_ns": impulse[0],
            "right_impulse_ns": impulse[1],
            "combined_impulse_ns": impulse[0] + impulse[1],
            "contact_force_peak_n": max(forces),
            "contact_force_rate_nps": values["force_rate"],
            "forbidden_contact_links": ["FORBIDDEN_NON_PALM_BOX_COLLISION"]
            if values["forbidden"]
            else [],
            "box_translation_m": values["box_translation"],
            "box_yaw_change_rad": values["box_yaw"],
            "box_linear_speed_mps": values["box_linear"],
            "box_angular_speed_radps": values["box_angular"],
            "root_height_m": values["root_height"],
            "root_tilt_deg": values["root_tilt"],
            "minimum_arm_joint_limit_margin_rad": values["joint_margin"],
            "arm_torque_ratio_max": values["torque_ratio"],
            "left_contact_loss_streak": int(current_state.loss_streak[0, 0]),
            "right_contact_loss_streak": int(current_state.loss_streak[0, 1]),
            "post_initial_reset_count": 0,
            "left_actual_surface_gap_m": gaps[0],
            "right_actual_surface_gap_m": gaps[1],
            "left_position_error_m": float(torch.linalg.vector_norm(values["position_error"][0])),
            "right_position_error_m": float(torch.linalg.vector_norm(values["position_error"][1])),
            "left_orientation_error_deg": math.degrees(
                float(torch.linalg.vector_norm(values["orientation_error"][0]))
            ),
            "right_orientation_error_deg": math.degrees(
                float(torch.linalg.vector_norm(values["orientation_error"][1]))
            ),
            "normalized_action": arm.raw_actions[0].detach().cpu().tolist(),
            "learned_correction_m": arm.correction_m[0].detach().cpu().tolist(),
            "nominal_displacement_m": arm.nominal_displacement_m[0].detach().cpu().tolist(),
            "applied_joint_residual_rad": arm.applied_residual[0].detach().cpu().tolist(),
            "box_push_commanded": False,
        }
        records.append(record)
        return record

    def bootstrap_record(phase: str, _step: int, metric) -> None:
        state_name = "STAND_SETTLE" if phase == "STAND_SETTLE" else "PRECONTACT"
        if not records or records[-1]["fsm_state"] != state_name:
            transitions.append({"frame": len(records) - 1, "state": state_name, "reason": None})
        append_record(state_name, metric)

    replay_reference, replay_audit = derive_precontact_reference(
        env, record_callback=bootstrap_record
    )
    stored_reference = torch.load(reference_path, map_location="cpu", weights_only=True)
    tensor_names = (
        "robot_root_state_relative",
        "robot_joint_position",
        "robot_joint_velocity",
        "box_root_state_relative",
        "arm_ik_target",
        "lower_hidden_state",
        "lower_cell_state",
        "previous_lower_policy_action",
    )
    replay_diffs = {
        name: float(torch.max(torch.abs(replay_reference[name] - stored_reference[name])))
        for name in tensor_names
    }
    replay_max_diff = max(replay_diffs.values())
    write_json(
        RUN / "precontact_reference_replay_comparison.json",
        {
            "schema_version": 1,
            "status": "DIAGNOSTIC_ONLY",
            "camera_enabled_bootstrap": bool(args.enable_cameras),
            "tensor_max_abs_diff": replay_diffs,
            "maximum_abs_diff": replay_max_diff,
            "scientific_gate": False,
        },
    )
    if not math.isfinite(replay_max_diff):
        raise RuntimeError("PRECONTACT_REFERENCE_REPLAY_NONFINITE")

    env.install_precontact_reference(stored_reference)
    env._s2_03t_terminal_snapshots.clear()
    initial_observation, _ = env.reset(seed=args.development_seed)
    installed_diffs = env.installed_reference_tensor_diffs(0)
    installed_max_diff = max(installed_diffs.values())
    arm = env.action_manager.get_term("arm_residual")
    lower = env.action_manager.get_term("frozen_lower_body")
    state = runtime_state(env)
    installed_metric = state.ensure()
    checks = {
        "reference_tensors_finite": all(math.isfinite(value) for value in installed_diffs.values()),
        "reference_tensors_within_tolerance": installed_max_diff <= 1.0e-5,
        "arm_processed_equals_reference": bool(torch.equal(arm.processed_actions, arm.reference)),
        "arm_raw_history_zero": bool(torch.count_nonzero(arm.raw_actions) == 0),
        "arm_previous_history_zero": bool(torch.count_nonzero(arm.previous_raw_actions) == 0),
        "arm_delta_history_zero": bool(torch.count_nonzero(arm.action_delta) == 0),
        "arm_correction_zero": bool(torch.count_nonzero(arm.correction_m) == 0),
        "arm_nominal_zero": bool(torch.count_nonzero(arm.nominal_displacement_m) == 0),
        "arm_contact_latch_clear": not bool(arm.contact_latched.any()),
        "arm_safety_latch_clear": not bool(arm.safety_latched.any()),
        "lower_public_action_empty": lower.raw_actions.numel() == 0,
        "lower_policy_matches_previous": bool(
            torch.equal(lower.policy_actions, lower.previous_policy_actions)
        ),
        "lower_processed_target_finite": bool(torch.isfinite(lower.processed_actions).all()),
        "contact_counters_zero": int(state.verify_count[0]) == int(state.hold_count[0]) == 0,
        "episode_length_zero": int(env.episode_length_buf[0]) == 0,
        "contacts_clear": not bool(installed_metric["contacts"].any()),
        "box_reference_zero": float(installed_metric["box_translation"][0]) == 0.0,
        "initial_policy_observation_shape": list(initial_observation["policy"].shape)
        == [1, ACTOR_OBSERVATION_DIM],
        "initial_critic_observation_shape": list(initial_observation["critic"].shape)
        == [1, CRITIC_OBSERVATION_DIM],
        "initial_observation_finite": bool(
            torch.isfinite(initial_observation["policy"]).all()
            and torch.isfinite(initial_observation["critic"]).all()
        ),
    }
    installed_valid = installed_max_diff <= 1.0e-5 and all(checks.values())
    write_json(
        RUN / "installed_reference_audit.json",
        {
            "schema_version": 1,
            "status": "PASS" if installed_valid else "INVALID",
            "reference_path": str(reference_path),
            "reference_sha256": args.reference_sha256,
            "tensor_max_abs_diff": installed_diffs,
            "maximum_abs_diff": installed_max_diff,
            "threshold": 1.0e-5,
            "checks": checks,
        },
    )
    if not installed_valid:
        raise RuntimeError(f"INSTALLED_PRECONTACT_REFERENCE_MISMATCH:{installed_max_diff}")

    if args.visual_evidence:
        visual_recorder = VisualEvidenceRecorder(
            RUN,
            env,
            controller_id=str(args.visual_evidence_id),
            checkpoint_path=None if checkpoint_path is None else str(checkpoint_path),
            checkpoint_sha256=None if checkpoint_path is None else args.checkpoint_sha256,
            checkpoint_iteration=args.checkpoint_iteration,
            seed=args.development_seed,
            reference_path=str(reference_path),
            reference_sha256=args.reference_sha256,
            frame_stride=args.visual_frame_stride,
        )
        precontact = append_record("PRECONTACT", installed_metric)
        visual_recorder.observe(
            precontact, keyframe="initial", actor_phase=True, force_video_frame=True
        )
        visual_recorder.mark_precontact(precontact)

    transitions.append({"frame": len(records) - 1, "state": "APPROACH_NORMAL", "reason": None})
    terminal_captured = [False]

    def state_name() -> str:
        current = runtime_state(env)
        metric = current.ensure()
        if bool(current.contact_verified[0]):
            return "ATTACHED_HOLD" if int(current.hold_count[0]) > 0 else "BILATERAL_CONTACT_VERIFY"
        return (
            "BILATERAL_CONTACT_VERIFY" if bool(metric["contacts"][0].any()) else "APPROACH_NORMAL"
        )

    def capture_terminal() -> None:
        if terminal_captured[0]:
            return
        record = append_record(state_name(), runtime_state(env).ensure())
        if visual_recorder is not None:
            visual_recorder.observe(
                record, keyframe="terminal", actor_phase=True, force_video_frame=True
            )
        elif camera is not None:
            rgb = camera.data.output["rgb"][0].detach().cpu().numpy()
            Image.fromarray(rgb[..., :3].astype("uint8")).save(RUN / "final.png")
        terminal_captured[0] = True

    env.visual_terminal_callback = capture_terminal

    wrapped = RslRlVecEnvWrapper(env, clip_actions=1.0)
    agent_cfg = S203TPPORunnerCfg()
    agent_cfg.resume = False
    agent_cfg.load_run = agent_cfg.load_checkpoint = None
    agent_cfg.load_optimizer = False
    runner = (
        OnPolicyRunner(wrapped, agent_cfg.to_dict(), log_dir=None, device=agent_cfg.device)
        if args.controller_id == "actor"
        else None
    )
    if checkpoint_path is not None:
        if runner is None:
            raise RuntimeError("ORIGINAL_CONTROLLER_RUNNER_MUST_BE_NONE")
        runner.load(str(checkpoint_path), load_optimizer=False)
    if runner is not None:
        policy = runner.get_inference_policy(device=env.device)
    else:
        # Legacy visual diagnostic preserved for comparison only.  It cannot be
        # selected, screened, or qualified by the redesign campaign.
        arm.set_bootstrap_mode(True)
        original_config = load_config(Path("configs/stage2/s2_03_attach_only.yaml").resolve())
        box_fixed_pos = env.scene["box"].data.root_link_pos_w.clone()
        box_fixed_quat = env.scene["box"].data.root_link_quat_w.clone()
        original_state = {
            "displacement": 0.0,
            "speed": 0.0,
            "acceleration": 0.0,
            "frozen": None,
        }
        object_x = torch.tensor([[1.0, 0.0, 0.0]], device=env.device)

        def original_policy(_observation):
            metric = runtime_state(env).ensure()
            if original_state["frozen"] is None and bool(metric["contacts"][0].all()):
                original_state["frozen"] = original_state["displacement"]
            maximum_distance = float(original_config["motion"]["maximum_approach_distance_m"])
            if (
                original_state["frozen"] is None
                and original_state["displacement"] < maximum_distance
            ):
                displacement, speed, acceleration = advance_rate_limited(
                    original_state["displacement"],
                    original_state["speed"],
                    original_state["acceleration"],
                    dt_s=float(original_config["controller"]["control_dt_s"]),
                    speed_limit_mps=float(original_config["motion"]["approach_speed_mps"]),
                    acceleration_limit_mps2=float(
                        original_config["motion"]["approach_acceleration_limit_mps2"]
                    ),
                    jerk_limit_mps3=float(original_config["motion"]["approach_jerk_limit_mps3"]),
                )
                original_state.update(
                    displacement=min(displacement, maximum_distance),
                    speed=speed,
                    acceleration=acceleration,
                )
            commanded = (
                original_state["frozen"]
                if original_state["frozen"] is not None
                else original_state["displacement"]
            )
            target_pos, target_quat = _target_pose_in_pelvis(env, box_fixed_pos, box_fixed_quat)
            _, object_quat_b = math_utils.subtract_frame_transforms(
                env.scene["robot"].data.root_link_pos_w,
                env.scene["robot"].data.root_link_quat_w,
                box_fixed_pos,
                box_fixed_quat,
            )
            object_x_b = math_utils.quat_apply(object_quat_b, object_x)
            desired_pos = target_pos + float(commanded) * object_x_b[:, None, :]
            palms = env.scene["hand_frames"]
            pos_error, orientation_error = math_utils.compute_pose_error(
                palms.data.target_pos_source.reshape(-1, 3),
                palms.data.target_quat_source.reshape(-1, 4),
                desired_pos.reshape(-1, 3),
                target_quat.reshape(-1, 4),
                rot_error_type="axis_angle",
            )
            commands = torch.cat(
                (
                    _clamp_norm(
                        pos_error.reshape(env.num_envs, 2, 3),
                        MAXIMUM_POSITION_CORRECTION_M,
                    ),
                    _clamp_norm(
                        orientation_error.reshape(env.num_envs, 2, 3),
                        MAXIMUM_ORIENTATION_CORRECTION_RAD,
                    ),
                ),
                dim=-1,
            )
            arm.set_bootstrap_commands(commands)
            if visual_recorder is not None:
                visual_recorder.set_original_commanded_normal_displacement(float(commanded))
            return torch.zeros((1, ACTOR_ACTION_DIM), device=env.device)

        policy = original_policy
    observation, _ = wrapped.get_observations()
    if tuple(observation.shape) != (1, ACTOR_OBSERVATION_DIM):
        raise RuntimeError(f"ACTOR_OBSERVATION_SHAPE_INVALID:{tuple(observation.shape)}")
    terminal_snapshot = None
    last_state = "APPROACH_NORMAL"
    for step in range(MAXIMUM_EPISODE_STEPS):
        with torch.inference_mode():
            action = policy(observation)
        if tuple(action.shape) != (1, ACTOR_ACTION_DIM) or not bool(torch.isfinite(action).all()):
            raise RuntimeError(f"ACTOR_ACTION_INVALID:{tuple(action.shape)}")
        observation, reward, done, _ = wrapped.step(action)
        if not bool(torch.isfinite(observation).all() and torch.isfinite(reward).all()):
            raise RuntimeError("EVALUATION_NONFINITE")
        if bool(done[0]):
            terminal_snapshot = env.pop_terminal_snapshot(0)
            if terminal_snapshot is None:
                raise RuntimeError("TERMINAL_SNAPSHOT_MISSING")
            if not terminal_captured[0]:
                raise RuntimeError("TERMINAL_PRE_RESET_CAPTURE_MISSING")
            break
        current_name = state_name()
        if current_name != last_state:
            transitions.append({"frame": len(records) - 1, "state": current_name, "reason": None})
            last_state = current_name
        record = append_record(current_name, runtime_state(env).ensure())
        if visual_recorder is not None:
            visual_recorder.observe(record, actor_phase=True)
        if (step + 1) % 25 == 0:
            print(f"PHASE=ACTOR_EVALUATION step={step + 1}", flush=True)
    if terminal_snapshot is None:
        raise RuntimeError("EVALUATION_TERMINAL_EVIDENCE_MISSING")

    terminal_state = "PASS" if terminal_snapshot["success"] else "FAIL"
    failed_reason = (
        None if terminal_state == "PASS" else failure_reason(terminal_snapshot, records[-1])
    )
    if failed_reason is not None:
        transitions.append({"frame": len(records) - 1, "state": "FAIL", "reason": failed_reason})
    outcome = {
        "schema_version": 1,
        "stage": "S2-03",
        "terminal_state": terminal_state,
        "failure_reason": failed_reason,
        "fsm_transitions": transitions,
        "left_contact_onset_frame": contact_onsets[0],
        "right_contact_onset_frame": contact_onsets[1],
        "bilateral_contact_onset_frame": bilateral_onset,
        "left_contact_loss_frames": contact_losses[0],
        "right_contact_loss_frames": contact_losses[1],
        "left_impulse_ns": terminal_snapshot["impulse_ns"][0],
        "right_impulse_ns": terminal_snapshot["impulse_ns"][1],
        "observed_frames": len(records),
        "box_push_commanded": False,
    }
    with (RUN / "trace.jsonl").open("w", encoding="utf-8") as stream:
        for record in records:
            stream.write(json.dumps(record, sort_keys=True, allow_nan=False) + "\n")
    write_json(RUN / "fsm_outcome.json", outcome)

    # Camera-free screening waives only the legacy final-image packaging bit.
    # All physical S2-03 contact, hold and safety gates use the same classifier.
    frozen_config = load_config(Path("configs/stage2/s2_03_attach_only.yaml").resolve())
    legacy_packaging_decision = classify_formal(
        frozen_config,
        records,
        outcome,
        final_image_present=bool(args.enable_cameras),
    )
    decision = classify_formal(
        frozen_config,
        records,
        outcome,
        final_image_present=bool(args.enable_cameras or args.mode == "screening"),
    )
    hold = [record for record in records if record["fsm_state"] == "ATTACHED_HOLD"]
    bilateral_records = [
        record for record in records if record["left_contact"] and record["right_contact"]
    ]
    bilateral_force_imbalances = [
        abs(float(record["left_force_n"]) - float(record["right_force_n"]))
        for record in bilateral_records
    ]
    result = {
        "schema_version": 1,
        "stage": "S2-03T",
        "task": "BILATERAL_ATTACH_ONLY",
        **decision,
        "authoritative_complete_before_teardown": True,
        "episode_count": 1,
        "observed_frames": len(records),
        "reset_count": 0,
        "post_initial_reset_count": 0,
        "camera_enabled": bool(args.enable_cameras),
        "camera_free_screening": bool(args.mode == "screening" and not args.enable_cameras),
        "classifier_image_compatibility_waiver": bool(
            args.mode == "screening" and not args.enable_cameras
        ),
        "legacy_packaging_decision_without_camera": legacy_packaging_decision,
        "screening_waiver_scope": "FINAL_IMAGE_PACKAGING_ONLY",
        "formal_s2_03_scientific_gates_unchanged": True,
        "terminal_trace_capture": "PRE_AUTO_RESET_PHYSICS_FRAME",
        "terminal_snapshot_capture": "S203TContactEnv._reset_idx_BEFORE_SUPER_RESET",
        "post_done_steps_executed": 0,
        "fsm_outcome": outcome,
        "terminal_snapshot": terminal_snapshot,
        "contact_occurred": any(r["left_contact"] or r["right_contact"] for r in records),
        "bilateral_contact_achieved": bool(terminal_snapshot["bilateral_contact_seen"]),
        "bilateral_verify_completed": int(terminal_snapshot["verify_steps"]) >= 20,
        "safe_hold_completed": int(terminal_snapshot["attached_hold_steps"]) >= 100,
        "attached_hold_observed_frames": len(hold),
        "box_push_commanded": False,
        "minimum_surface_gap_m": terminal_snapshot["minimum_surface_gap_m"],
        "maximum_force_n": terminal_snapshot["maximum_force_n"],
        "mean_bilateral_force_imbalance_n": (
            sum(bilateral_force_imbalances) / len(bilateral_force_imbalances)
            if bilateral_force_imbalances
            else None
        ),
        "maximum_bilateral_force_imbalance_n": (
            max(bilateral_force_imbalances) if bilateral_force_imbalances else None
        ),
        "minimum_root_height_m": min(float(r["root_height_m"]) for r in records),
        "minimum_arm_joint_limit_margin_rad": min(
            float(r["minimum_arm_joint_limit_margin_rad"]) for r in records
        ),
        "forbidden_contact_links": sorted(
            {link for r in records for link in r["forbidden_contact_links"]}
        ),
        "checkpoint": None if checkpoint_path is None else str(checkpoint_path),
        "checkpoint_sha256": None if checkpoint_path is None else args.checkpoint_sha256,
        "checkpoint_iteration": args.checkpoint_iteration,
        "development_seed": args.development_seed,
        "evaluation_mode": args.mode.upper(),
        "controller_id": args.controller_id,
        "reference_path": str(reference_path),
        "reference_sha256": args.reference_sha256,
        "action_dim": ACTOR_ACTION_DIM,
        "actor_observation_dim": ACTOR_OBSERVATION_DIM,
        "critic_observation_dim": CRITIC_OBSERVATION_DIM,
        "deterministic_actor_mean": True,
        "planner_started": False,
    }
    scientific_status = str(result["status"])
    scientific_reason = str(result["primary_reason"])
    write_json(RUN / "evaluation_summary.json", result)
    write_json(RUN / "result.json", result)
    metadata = {
        "schema_version": 1,
        "stage": "S2-03T",
        "mode": args.mode.upper(),
        "development_seed": args.development_seed,
        "controller_id": args.visual_evidence_id or args.controller_id,
        "checkpoint": result["checkpoint"],
        "checkpoint_sha256": result["checkpoint_sha256"],
        "checkpoint_iteration": args.checkpoint_iteration,
        "deterministic_actor_mean": True,
        "reference_path": str(reference_path),
        "reference_sha256": args.reference_sha256,
        "reference_replay_max_abs_diff": replay_max_diff,
        "reference_replay_gate": "DIAGNOSTIC_ONLY",
        "installed_reference_max_abs_diff": installed_max_diff,
        "installed_reference_tensor_diffs": installed_diffs,
        "reference_tensor_diffs": replay_diffs,
        "precontact_replay_audit": replay_audit,
        "terminal_snapshot": terminal_snapshot,
        "post_initial_reset_count": 0,
        "box_push_commanded": False,
        "planner_started": False,
    }
    write_json(RUN / "actor_evaluation_metadata.json", metadata)

    if visual_recorder is not None:
        try:
            visual = visual_recorder.finalize(
                records=records,
                terminal_state=scientific_status,
                failure_reason=failed_reason,
                result_primary_reason=scientific_reason,
                installed_reference_max_abs_diff=installed_max_diff,
                reference_replay_max_abs_diff=replay_max_diff,
            )
            shutil.copyfile(RUN / "frame_terminal.png", RUN / "final.png")
            result["visualization_status"] = visual["visualization_status"]
            write_json(RUN / "result.json", result)
        except BaseException as visual_exc:
            print(f"VISUALIZATION_INVALID reason={visual_exc!r}", file=sys.stderr, flush=True)
            visual_recorder.close_incomplete(reason=f"VISUALIZATION_EXCEPTION:{visual_exc!r}")
    write_json(
        RUN / "runner_status.json",
        {
            "status": "COMPLETE" if scientific_status in {"PASS", "FAIL"} else "INVALID",
            "phase": "EVIDENCE_READY",
            "scientific_status": scientific_status,
            "scientific_primary_reason": scientific_reason,
        },
    )
except BaseException as exc:
    traceback.print_exception(type(exc), exc, exc.__traceback__, file=sys.stderr)
    write_json(
        RUN / "result.json",
        {
            "schema_version": 1,
            "stage": "S2-03T",
            "status": "INVALID",
            "primary_reason": "IMPLEMENTATION_EXCEPTION",
            "authoritative_complete_before_teardown": True,
            "error": repr(exc),
        },
    )
    write_json(
        RUN / "runner_status.json",
        {
            "status": "INVALID",
            "primary_reason": "IMPLEMENTATION_EXCEPTION",
            "error": repr(exc),
        },
    )
    scientific_status = "INVALID"
finally:
    if visual_recorder is not None and not visual_recorder.finalized:
        visual_recorder.close_incomplete(reason="EVALUATOR_EXCEPTION_BEFORE_VISUAL_FINALIZE")
    if env is not None:
        env.close()
    simulation_app.close()

raise SystemExit(0 if scientific_status in {"PASS", "FAIL"} else 2)
