...
# The caller supplies a new, empty run root. Keeping the contract external
# makes accidental continuation of an old evidence directory impossible.
...
sha256sum \
  "${SOURCE_ROOT}/scripts/stage2_isaac/run_s2_03t_chest_prepose.py" \
  "${SOURCE_ROOT}/src/g1_access_push/sim/stage2/s2_03t_chest_prepose_env_cfg.py" \
  "${SOURCE_ROOT}/src/g1_access_push/stage2/s2_03t_chest_prepose_contract.py" \
  > "${RUN_ROOT}/source_sha256_manifest.txt"

start_epoch=$(date +%s)
printf '%s\n' "${start_epoch}" > "${RUN_ROOT}/start_epoch_seconds.txt"
set +e
python "${SOURCE_ROOT}/scripts/stage2_isaac/run_s2_03t_chest_prepose.py" \
  --run-root "${RUN_ROOT}" \
