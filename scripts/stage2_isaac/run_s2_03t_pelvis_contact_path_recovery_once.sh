...
sha256sum \
  "${SOURCE_ROOT}/scripts/stage2_isaac/run_s2_03t_pelvis_contact_path_recovery.py" \
  "${SOURCE_ROOT}/scripts/stage2_isaac/run_s2_03t_pelvis_contact_path_recovery_entry.py" \
  "${SOURCE_ROOT}/scripts/stage2_isaac/s2_03t_pelvis_contact_campaign.py" \
  "${SOURCE_ROOT}/src/g1_access_push/stage2/s2_03t_pelvis_contact_runtime.py" \
  "${SOURCE_ROOT}/src/g1_access_push/stage2/s2_03t_pelvis_contact_path_recovery_contract.py" \
  "${SOURCE_ROOT}/configs/stage2/s2_03t_safe_chest_waypoint_path.yaml" \
  > "${RUN_ROOT}/source_sha256_manifest.txt"

start_epoch=$(date +%s)
printf '%s\n' "${start_epoch}" > "${RUN_ROOT}/start_epoch_seconds.txt"
set +e
python "${SOURCE_ROOT}/scripts/stage2_isaac/run_s2_03t_pelvis_contact_path_recovery_entry.py" \
  --run-root "${RUN_ROOT}" \
