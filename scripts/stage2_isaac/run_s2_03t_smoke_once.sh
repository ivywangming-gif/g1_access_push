...
RC=${PIPESTATUS[0]}
python -c "import json,sys; payload=json.loads(open(sys.argv[1], encoding=\"utf-8\").read()); status=payload[\"status\"]; print(f\"AUTHORITATIVE_STATUS={status} PATH={sys.argv[1]}\", flush=True); raise SystemExit(0 if status in sys.argv[2:] else 2)" "${RUN_ROOT}/contract_smoke_result.json" PASS 2>&1 | tee -a "${EXTERNAL_LOG}"
